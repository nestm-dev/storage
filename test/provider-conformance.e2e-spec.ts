...
function parseCustomCapabilityTokens(
  ...
  for (const token of parsed) {
    if (!known.has(token)) {
      throw new TypeError(
        `STORAGE_CONFORMANCE_CUSTOM_CAPABILITIES contains unknown token ${JSON.stringify(token)}.`,
      );
    }
    if (tokens.has(token as CustomCapabilityToken)) {
      throw new TypeError(
        `STORAGE_CONFORMANCE_CUSTOM_CAPABILITIES repeats token ${JSON.stringify(token)}.`,
      );
    }
